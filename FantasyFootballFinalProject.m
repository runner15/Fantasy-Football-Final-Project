...
for l=1:length(allPlayers.players.player)
    players(l).id=allPlayers.players.player{l,1}.id;
    players(l).info.name=allPlayers.players.player{l,1}.name;
    players(l).info.position=allPlayers.players.player{l,1}.position;
    players(l).info.team=allPlayers.players.player{l,1}.team;
end
...
for k=1:length(roster.rosters.franchise)
    for l=1:length(roster.rosters.franchise(k).player)
        allRostersMat(l,k) = str2double(roster.rosters.franchise(k).player(l,1).id);
